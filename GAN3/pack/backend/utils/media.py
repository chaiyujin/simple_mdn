...
def get_mfcc(audio_path):
    ...
    assert(fs == 16000)
    left = 48080 - audio.shape[0]
    zeros = numpy.zeros((left))
    audio = numpy.append(audio, zeros)
    zeros = numpy.zeros((80))
    audio = numpy.append(zeros, audio)
    print(audio.shape)
    mfcc_feat = mfcc(
        audio, fs, 0.02, 0.01, 16, 26, 512
    )
    return mfcc_feat
...
def process_media(video_path, config, show=False, fbx=False, clear_old=False):
    ...
    if a_frames <= 0 or a_frames != AUDIO_FRAMES:
