...
def generator(x, z, config, scope='Gen', reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # input 75x16, 1
        enc.conv_layer(16, [1, 3], [1, 2], 'lrelu')
        enc.conv_layer(32, [1, 3], [1, 2], 'lrelu', True)
        enc.conv_layer(64, [3, 3], [2, 2], 'lrelu', True)
        enc.conv_layer(128, [3, 1], [2, 1], 'lrelu', True)
        bottleneck = enc(x)
        b_s = [int(d) for d in bottleneck.get_shape()]
        z = tf.reshape(z, [b_s[0], b_s[1], b_s[2], -1])
        bottleneck = tf.concat([bottleneck, z], axis=3)
        dec.deconv_layer(64, [1, 3], [1, 2], 'lrelu', True)
        dec.deconv_layer(32, [2, 3], [1, 2], 'lrelu', True)
        dec.deconv_layer(16, [2, 2], [1, 2], 'lrelu', True)
        lin = tf.reshape(dec(bottleneck), [x_s[0] * config['time_steps'], -1])
        logits = tflayers.fully_connected(
            lin, config['expr_dims'], None,
            weights_initializer=tf.random_normal_initializer(0, 0.02)
        )
        logits = tf.reshape(
            logits, [x_s[0], config['time_steps'], config['expr_dims'], 1]
        )
...
def discriminator(x, y, scope='Dis', reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        tmp = ConvNet('Conv_X')
        tmp.conv_layer(4, [3, 1], [2, 1], 'lrelu')
        tmp.conv_layer(8, [3, 1], [2, 1], 'lrelu')
        x = tmp(x)
...
class GAN():
    def __init__(self, scope='AudioGAN', config=default_config):
        ...
        # the shapes
        self.x_shape = [self._b_size, self._img_h * 4, self._img_w, 1]
