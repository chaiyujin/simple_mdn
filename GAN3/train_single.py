...
def generator(x, y_s, scope='gen', reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        lin = tf.reshape(dec(enc(x)), [y_s[0] * y_s[1], -1])
        logits = tflayers.fully_connected(
            lin, 19, None,
            weights_initializer=tf.random_normal_initializer(0, 0.02)
        )
        logits = tf.reshape(
            logits, [y_s[0], y_s[1], 19, 1]
        )
...
if __name__ == '__main__':
    ...
    # net
    batch_size = 32
    x_s = [batch_size, 300, 16, 1]
    y_s = [batch_size, 75, 19, 1]
    x = tf.placeholder(tf.float32, x_s)
    y = tf.placeholder(tf.float32, y_s)
    y_fake, y_fake_logits = generator(x, y_s)
    print(y.get_shape())
    print(y_fake_logits.get_shape())
